	...
	public function accessRules() {
		return array (
				array (
						...
						'actions' => array ('search','accountVerify','create','login','view','recover','download','signup',
								'search','check','report','reportCauses','reports','logout','update','profile','changepassword',
								'posts','nearByPosts','nearByUsers','addUser','getAlluser','createLocation','viewAllLocation','loginFB' 
						),
						'users' => array ('*') 
				),
				array (
						'allow', // allow authenticated user to perform
						'actions' => array ('userFriends','logout','update','profile','updateProfileImg','changepassword','start',
								'invite','inviteFriends','getInvitedList','updateInvitation','getInvitee','addUser','getAlluser' 
						),
						'users' => array ('@') 
				),
				array (
	...
	public function actionProfile($id = null) // UserDetail
{
	...
	$arr = array ('controller' => $this->id,'action' => $this->action->id,'status' => 'NOK');
		$model = new LoginForm ();
			...
			$loginByFB = true;
			if ($this->authenticate( $model, $loginByFB )) {		
				...
				//while login with fb, already exiting email will save facebook_id
				if(!$user->facebook_id){
					$user->facebook_id = $_POST ['LoginForm'] ['username']; 				
					$user->save();
				}
				else 
					$user->facebook_id = $_POST ['LoginForm'] ['username'];
	 ...
	 * @return boolean
	 */
	public function authenticate($user, $loginByFB = false) { // var_dump($user->username);die;
	
		$identity = new UserIdentity ( $user->username, $user->password );
		$identity->authenticateApi ($loginByFB);
		...
		if (isset ( $_POST ['Invitation'] )) {
			foreach ( $_POST ['Invitation'] as $invite ) {
				...
				if (! $search_email) {
					...
					// $_POST['Invitation']['email'];
					// $_POST['Invitation']['name'];
	...
	public function actionNearByPosts($radius = null, $lat = null, $long = null) {
		$id = Yii::app ()->user->id;
		if ($id == null)
			$id = $_REQUEST ['User'] ['id'];
