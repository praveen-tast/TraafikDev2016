...
class UserController extends GxController {
	...
	public function accessRules() {
		return array(
				array('allow',  // allow all users to perform 'index' and 'view' actions
						'actions'=>array('search','accountVerify','create','login','view','recover','download','signup','search', 'check',
								'report', 'reportCauses', 'reports',
								'logout', 'update','profile','changepassword',
								'addUser','getAlluser'
						),
						'users'=>array('*'),
				),
				array('allow', // allow authenticated user to perform
						'actions'=>array( 'userFriends','logout', 'update','profile','updateProfileImg','changepassword','start','invite', 
								'inviteFriends','getInvitedList', 'updateInvitation','getInvitee','addUser','getAlluser'),
						'users'=>array('@'),
				),
	...
	public function actionInvite()
	{
		...
		{			
			$search_invite = Invitation::model()->findByAttributes(array('email' => $_POST['Invitation']['email'] , 'create_user_id' => Yii::app()->user->id));
			if(!$search_invite)
			{
	...
	public function actionInviteFriends()
	{
		...
		if(isset($_POST['Invitation']))
		{
			...
			{
				$search_email = Invitation::model()->findByAttributes(array('email' => $invite['email'] ));
				if(!$search_email){
					$invitation = new Invitation();
	...
	{
		$arr = array('controller'=>$this->id, 'action'=>$this->action->id,'status' =>'NOK');
		$results = Invitation::model()->findAllByAttributes(array('create_user_id' => Yii::app()->user->id ));
		if($results)
		{
			$json_entry = array();
	...
	public function actionUpdateInvitation()
	{
		...
		{
			$invitation = Invitation::model()->findByAttributes(array('email' => $_POST['Invitation']['email'] ));
			if($invitation)
			{
	...
	public function actionGetInvitee()
	{
		...
		if(isset($_POST['Invitation']))
		{			
			$invitation = Invitation::model()->findByAttributes(array('email' => $_POST['Invitation']['email'], 'type_id' => Invitation::TYPE_FACEBOOK));
	...
	public function actionAddUser($id)
	{
		...
		{
			$search_group = Group::model()->findByAttributes(array('name'=>Yii::app()->user->name , 'user_id'=>Yii::app()->user->id));
			...
			{	
				...
				if($group->save())
				{					
					$search_usergroup = UserGroup::model()->findByAttributes(array('user_id'=>$id, 'group_id'=>$group->id));
					if(!$search_usergroup)
					{					
			...
			{	
				$search_usergroup = UserGroup::model()->findByAttributes(array('user_id'=>$id, 'group_id'=>$search_group->id));
				if(!$search_usergroup)
				{		
